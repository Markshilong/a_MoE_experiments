...
shilong: Adapted from single_task_fine_tune.py
...
def parse_args():
    ...
    parser.add_argument(
        ...
        "--deepspeed",
        action="store_true",
        help="If passed, using deepspeed zero",
    )
    # my -----
    parser.add_argument(
        "-d",
...
def main():
    now_time = time()
    print(f" ---------------------------Start Timer--------------------")
    print(" ---------- Start .py at " + datetime.now().strftime("%Y-%m-%d %H:%M:%S"))
    args = parse_args()
    set_seed(args.seed)
    ...
    if args.deepspeed:
        ...
        ds_config = {
            ...
            "zero_optimization": {
                ...
                "offload_optimizer": {
                    "device": "nvme",
                    "nvme_path": "/home/shilonglei/OOC/nvme_offload/optimizer_offload",
                    "pin_memory": True,
                    "buffer_count": 4,
                    "fast_init": False
                },
    ...
    if args.pad_to_max_length:
        ...
        eval_collator = DataCollatorForMultipleChoice(
            tokenizer, pad_to_multiple_of=8 #(8 if accelerator.use_fp16 else None)
        )
    ...
    if args.deepspeed:
        ...
        # ds_engine, optimizer, _, _ = deepspeed.initialize(model=model, optimizer=optimizer, lr_scheduler=lr_scheduler, config_params=ds_config)
        nvtx.range_pop()
    # Metrics
    ...
    last_time = now_time
    now_time = time()
    print(f" --- To here, TimeSpent = {now_time - last_time}s")
    ...
    print(" ---------- Start Training at " + datetime.now().strftime("%Y-%m-%d %H:%M:%S"))
    result_table = []
    for epoch in range(1, args.num_train_epochs+1):
        if args.deepspeed:
            ds_engine.module.train()
            for step, batch in enumerate(train_dataloader):
                batch = batch.to(device=local_rank)
                print(f"!!! Shape = {batch['input_ids'].shape}")
                # # ----
                # data = batch.data  # Access the input tensors for the batch
                # print(len(data))
                # # print(data)
                # # ----
                ...
                last_time = now_time
                now_time = time()
                nvtx.range_push(f"Training step {step} epoch {epoch}")

                nvtx.range_push(f"Forward()")
                outputs = ds_engine(**batch)
                nvtx.range_pop()

                loss = outputs.loss

                nvtx.range_push(f"Backward()")
                ds_engine.backward(loss)
                nvtx.range_pop()

                nvtx.range_push(f"ds_engine.step()")
                ds_engine.step()
                # optimizer.step()
                # lr_scheduler.step()
                # optimizer.zero_grad()
                ...
                last_time = now_time
                now_time = time()
                tqdm.write(f"epoch = {epoch}, step = {global_steps}, loss = {loss}, TimeSpent = {now_time - last_time}s")
                # if args.wandb_proj and accelerator.is_main_process:
    ...
    # End training loop
    print(" ---------- End Training at " + datetime.now().strftime("%Y-%m-%d %H:%M:%S"))
